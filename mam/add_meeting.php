...
$(function() {
	  ...
	  function my_function2(emp_id,fullname)
      {

		 var exists = false; 
         $('#mam_join option').each(function(){
            if (this.value == emp_id) {
              exists = true;
            } 
         });
		 ...
		 if(!exists)
		 {
		      $("#mam_join").prepend("<option value='" + emp_id + "' selected>" + fullname + "</option>");
              $("#mam_join").selectmenu('refresh');
		 }
      }
	  
	  
	  send_value2 = my_function;
	  send_value4 = my_function2;
...
function send_value3(emp_id_tmp,fullname_tmp)
{
	send_value4(emp_id_tmp,fullname_tmp);
}
...
<script type="text/javascript">
                 $(document).ready(function () {
                    $('#radio-mini-1').click(function () {
                       $('#test_div').hide('fast');
                    });
					$('#radio-mini-2').click(function () {
                       $('#test_div').hide('fast');
                    });
                    $('#radio-mini-3').click(function () {
                      $('#test_div').show('fast');
                    });
					$('#radio-mini-4').click(function () {
                      $('#test_div2').hide('fast');
                    });
					$('#radio-mini-5').click(function () {
                      $('#test_div2').show('fast');
                    });
                 });
</script>
...
<div data-role="page" id="one">
	...
	<div data-role="header" data-theme="b" data-position="fixed">
	    <a href="javascript:window.location='../index.html?uid=<?=$uid; ?>';" data-icon="back">Back</a>
		<h1>การสร้างการนัดหมาย</h1>
	</div><!-- /header -->

	<div data-role="content" >

<form action="add_meeting_in.php" method="post" data-ajax="false" onsubmit="return checkform(this);" enctype="multipart/form-data">	
...
<option value="07:30:00">07:00</option>		
...
<div data-role="fieldcontain">
	...
	<fieldset data-role="controlgroup" data-type="horizontal">
	<input type="radio" name="join_status" id="radio-mini-4" value="1" checked="checked" />
    	<label for="radio-mini-4">ไม่ระบุรายชื่อผู้เข้าประชุม</label>

	<input type="radio" name="join_status" id="radio-mini-5" value="2" />
    	<label for="radio-mini-5">ระบุรายชื่อผู้เข้าประชุม</label>
    </fieldset>	
	
	<div id="test_div2" style='display:none;'>			  
	<br>
	
	<select name="mam_join[]" id="mam_join" multiple="multiple" data-native-menu="false">
			<option value="">รายชื่อผู้เข้าประชุม</option>
	</select>
	<br>
    <ul id="autocomplete2" data-role="listview" data-inset="true" data-filter="true" data-filter-placeholder="ค้นหารายชื่อผู้เข้าประชุม" data-filter-theme="d"></ul>
    </div>
...
<div data-role="fieldcontain">
    ...
    <input type="file" name="filesattach_name" id="filesattach_name" value="" />
</div>
